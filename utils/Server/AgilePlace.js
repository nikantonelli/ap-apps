...
class AgilePlace {
	...
	getHost() {
		return "https://" + process.env.NUI_HOST
	}

	getCardUrl(id) {
		return "https://" + process.env.NUI_HOST + "/card/" + id
	}
	
	getBoardUrl(id) {
		return "https://" + process.env.NUI_HOST + "/board/" + id
	}
	
	async xfr(params) {

		if (!(Boolean(process.env.NUI_HOST) && (Boolean(process.env.NUI_API_KEY) || (Boolean(process.env.NUI_USER_NAME) || Boolean(process.env.NUI_USER_PASS))))) return null;

		if (!Boolean(params.raw)) {
			params.baseUrl = "https://" + process.env.NUI_HOST + "/io"
		} else {
			params.baseUrl = "https://" + process.env.NUI_HOST
		}
		...
		if (params.encoding) {
			headers["Accept-Encoding"] = params.encoding;
		}

		if (process.env.NUI_API_KEY) {
			headers["Authorization"] = "Bearer " + process.env.NUI_API_KEY;
		}
		else {
			...
			var token = process.env.NUI_USER_NAME + ":" + process.env.NUI_USER_PWD;
