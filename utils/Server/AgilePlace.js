...
class AgilePlace {
	...
	async xfr(params) {
		...
		if (!Boolean(params.raw)) {
			params.baseUrl = "https://" + process.env.AGILEPLACE + "/io"
		} else {
			params.baseUrl = "https://" + process.env.AGILEPLACE
		}
		...
		if (process.env.AGILEKEY) {
			headers["Authorization"] = "Bearer " + process.env.AGILEKEY;
		}
		else {
