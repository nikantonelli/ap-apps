...
const Board = ({ board, active, host }) => {
	...
	const fcRef = useRef(fetchCount);

	async function getTopLevel(host, params) {
		var root = {
					id: 'root',
					children: []
				}
	...
	function childrenOf(host, cards) {
		console.log("asking for children of: ", _.map(cards, (card) => card.id))
		forEach(cards, (card) => {
			getChildren(host, card).then(async (result) => {
				var children = await result.json()
				console.log(`got:  ${_.map(children, (card) => card.id)} for card.id: ${card.id}`)
				card.children = children.cards
				childrenOf(host, card.children)
				console.log(card.children)
			})
		})
	}

	function getChildren(host, card) {
		var params = {
			mode: "GET",
	...
	useEffect(() => {
		...
		if (fetchActive) {
			...
			getTopLevel(host, params).then((result) => {
				var clonedData = JSON.parse(JSON.stringify(result))
